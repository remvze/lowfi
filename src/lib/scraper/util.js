    ...
    switch (type) {
      ...
      case 'playlist':
        return (
          ...
          (url.match(/soundcloud.app.goo.gl/) &&
            url.split('/').pop().length === 5)
        );
      case 'track':
        return (
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (res.status !== 200) {
          ...
          if (hasError)
            return reject(new Error(`[Code: ${res.status}] ${hasError}`));
          return reject(
            new Error(
              `[Code: ${res.status}] Rejected with status code "${res.status}"!`,
            ),
          );
        }
        res = await res.json();
